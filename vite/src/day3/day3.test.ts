...
function toBitMask(input: string): number {
    let mask = 0;
    for (const char of input) {
        const code = toCharCode(char);
        mask |= Math.pow(2, code - 1);
    }
    return mask;
}
...
describe("day 3", () => {
    ...
    describe.skip("to bit mask", () => {
        ...
        test_cases.forEach(([input, expected]) => {
            it(`bit mask of '${input}' = ${expected}`, () => {
                expect(toBitMask(input)).toBe(expected);
            })
        })
    })
