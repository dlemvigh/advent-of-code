// const red = "R8,U5,L5,D3";
// const blue = "U7,R6,D4,L4";
// const red = "R75,D30,R83,U83,L12,D49,R71,U7,L72";
// const blue = "U62,R66,U55,R34,D71,R55,D58,R83";
// const red = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51";
// const blue = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7";
const red =
...
function lineIntersects(a, b) {
  ...
  if (
    ...
    // console.log(v, h);
    // console.log("===");
    return [v.start[0], h.start[1], v, h];
  }
}

function wireToLines(wire) {
  const lines = [];
  let pos = [0, 0];
  let steps = 0;

  wire.forEach((segment, index) => {
    const dir = segment[0];
    const dist = Number(segment.substr(1));
    let start, end;
    if (dir === "R" || dir === "U") {
      start = pos;
      end = move(segment, pos);
    } else {
      end = pos;
      start = move(segment, pos);
    }
...
function findIntersections() {
  ...
  for (let i = 0; i < redLines.length; i++) {
    for (let j = 0; j < blueLines.length; j++) {
      ...
      const r = redLines[i];
      const b = blueLines[j];
      const inter = lineIntersects(r, b);
      if (inter) {
...
function calcFinalSteps([x, y, a, b]) {
  const calcFinal = pos => Math.abs(pos[0] - x) + Math.abs(pos[1] - y);
  const aFinal = calcFinal(a.pos);
  const bFinal = calcFinal(b.pos);
  //   console.log("steps", a.steps, b.steps, aFinal, bFinal);
  const steps = a.steps + aFinal + b.steps + bFinal;
  return steps;
}

function move(segment, pos) {
  const { dir, dist } = parseSegment(segment);
  return [pos[0] + dir[0] * dist, pos[1] + dir[1] * dist];
}

function allMoves(wire) {
  let pos = [0, 0];
  const moves = [pos];
